...
# Directory path
directory_path = "/Volumes/databricks_1j4iyuf/llm/pdf_vol/"
...
pdf_volume_path = directory_path #  Specify the path to the PDF volume directory

# Get the list of already processed PDF files from the Delta table
processed_files = spark.sql(f"SELECT DISTINCT file_name FROM databricks_1j4iyuf.llm.docs_track").collect()
...
# MAGIC %sql
# MAGIC insert into databricks_1j4iyuf.llm.docs_text (text)
...
spark.sql("""
    INSERT INTO databricks_1j4iyuf.llm.docs_track
    SELECT * FROM temp_table
    WHERE NOT EXISTS (
        SELECT 1 FROM databricks_1j4iyuf.llm.docs_track
        WHERE temp_table.file_name = databricks_1j4iyuf.llm.docs_track.file_name
    )
""")
